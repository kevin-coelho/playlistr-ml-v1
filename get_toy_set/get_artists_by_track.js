...
const main = async () => {
	try {
		...
		const artist_ids = arrayToBatches(toy_set.reduce((artist_ids, current_playlist) => {
		...
		Promise.map(artist_ids, id_array => Promise.all([ id_array, api_instance.request(getArtistConfig(id_array))])
			...
			.then(results => results.reduce((result_obj, batch_result) => {
				...
				//console.log(batch_result[1]);
					batch_result[0].forEach((id, idx) => {
						result_obj.success_ids[id] = batch_result[1].artists[idx];
					});
				}
				return result_obj;
			}, {}))
