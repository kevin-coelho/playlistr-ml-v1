	...
	down: (queryInterface, Sequelize) => {
		...
		*/
		...
		return queryInterface.bulkDelete('playlist_track', {
			[Op.or]: filtered.map(playlist_track => ({
				trackId: playlist_track.trackId,
				playlistId: PlaylistTrack.playlistId,
			})),
		}, {}).then(() => console.log(`Removed toy set playlist track linkings. Count: ${chalk.red(filtered.length)}`));
	}
};