...
//			.pipe(JSONStream.parse('.*.tracks.*.track.id'))
			...
			.pipe(es.through(async function write(playlist_track_arr) {
				if (!this.paused) this.pause();
				const track_ids = playlist_track_arr.map(playlist_track => playlist_track.track.id).filter(id => id);
				const results = await Promise.map(track_ids, track_id => Promise.all([track_id, api_instance.request(getTrackAudioAnalysisConfig(track_id))])
					.catch(err => {
						failed_results.push(track_id);
						console.error(`[${chalk.red(track_id)}] Request failed...`);
						return Promise.resolve([track_id, false]);
					}), { concurrency: 2 });
				results.forEach(result => {
					if (result[1]) {
						...
						delete result[1].track.synchstring;
						delete result[1].track.rhythmstring;
						const analysis_obj = { track: result[1].track };
						const stringified = `${sep}"${result[0]}": ${JSON.stringify(analysis_obj)}`;
						if (!sep) sep = ',';
						count = count + 1;
						console.log(`Writing analysis: ${chalk.yellow(result[0])}`);
						console.log(`Completed ${count}. Avg process time: ${((now('milli') - startTime) / count).toFixed(2)} ms`);
						this.emit('data', stringified);
					} else {
						this.emit('data', '');
					}		
				});
