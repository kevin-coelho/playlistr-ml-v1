// ENVIRONMENT
require('dotenv').config();

// DEPENDENCIES
const axios = require('axios');
const Promise = require('bluebird');
const fs = Promise.promisifyAll(require('fs'));
const moment = require('moment');
const btoa = require('btoa');
const PrettyError = require('pretty-error');
const pe = new PrettyError();
const chalk = require('chalk');
const qs = require('querystring');
const path = require('path');

// CONSTANTS
const resource_dir = 'api_manager'
const token_file = path.join(resource_dir, 'spotify_token.json');
const ERR_TYPES = {
	'bad_write': 'WriteError'
}
const SPOTIFY_API_URL = 'https://api.spotify.com/v1';
const TIMEOUT = 5000;
const api_instance = axios.create({
	baseURL: SPOTIFY_API_URL,
	timeout: TIMEOUT
});

// GLOBALS
let current_token = null;

// setup resource folder
function setupResource() {
	if (!fs.existsSync(resource_dir))
		return fs.mkdirAsync(resource_dir)
				 .then(() => resource_dir);
	return Promise.resolve(resource_dir);
}

// get access token
function getToken() {
	if (fs.existsSync(token_file)) {
		return fs.readFileAsync(token_file)
			.then(data => JSON.parse(data))
			.then(token_obj => {
				// token expired, get new one
				if (moment(token_obj.expiration).isBefore(moment())) return tokenRequest();
				else return Promise.resolve(token_obj);
			});
	} else return tokenRequest();
}

// send request to spotify to get a new access token and write to file
function tokenRequest() {
	return api_instance.request(tokenRequestConfig())
		.then(res => res.data)
		.then(token_obj => {
			token_obj.expiration = moment().add(token_obj.expires_in, 'seconds');
			return fs.writeFileAsync(token_file, JSON.stringify(token_obj))
				.then(() => token_obj);
		})
		.catch(err => {
			console.error(pe.render(err));
			if (err.response) {
				// The request was made and the server responded with a status code
				// that falls out of the range of 2xx
				console.error(err.response.data);
				console.error(err.response.status);
				console.error(err.response.headers);
			}
			return Promise.reject('Token request failed');
		});
}

// config for token request
function tokenRequestConfig() {
	const auth_header = btoa(`${process.env.SPOTIFY_CLIENT_ID}:${process.env.SPOTIFY_CLIENT_SECRET}`);
	return {
		url: 'https://accounts.spotify.com/api/token',
		method: 'post',
		headers: {
			'Authorization': `Basic ${auth_header}`,
			'content-type': 'application/x-www-form-urlencoded',
		},
		data: qs.stringify({
			grant_type: 'client_credentials'
		})
	};
}

// interceptor to add token to the beginning of all requests
const interceptor = (request) => {
	const current_token = api_instance.token_obj;
	if (current_token) {
		const get_token = ((token_obj) => {
			if (moment(token_obj.expiration).isBefore(moment())) return tokenRequest();
			return Promise.resolve(token_obj);
		})(current_token);		
		return get_token.then(token_obj => {
			api_instance.token_obj = token_obj;
			request.headers.Authorization = `Bearer ${token_obj.access_token}`;
			return request;
		});
	}
	return request;
}

module.exports = (() => {
	console.log(chalk.yellow('Setting up Spotify api instance...'));
	return setupResource()
		.then(() => getToken())
		.then(token_obj => {
			api_instance.token_obj = token_obj;
			api_instance.interceptors.request.use(interceptor);
			console.log(chalk.yellow('Added token to api...'));
			return Promise.resolve(api_instance);
		});
})();